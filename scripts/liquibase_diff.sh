...
liquibase --changeLogFile="/changelogs/migration/$TS-$CHANGELOG_FILE" diffChangeLog \
  --referenceUrl=$CONNECTION_STRING \
  --referenceUsername=$DB_USER \
  --referencePassword=$DB_PASS

echo "Diff generated into: /changelogs/migration/$TS-$CHANGELOG_FILE"
...
if [ ! -z "$LIQUIBASE_INCLUSION_FILE" ] ; then
  if [ -f "/changelogs/$LIQUIBASE_INCLUSION_FILE" ] ; then
    ...
    sed -i "
      /<\/databaseChangeLog>/ i\
      <include relativeToChangelogFile='true' file=\"/migration/$TS-$CHANGELOG_FILE\" />" "/changelogs/$LIQUIBASE_INCLUSION_FILE"
  else
